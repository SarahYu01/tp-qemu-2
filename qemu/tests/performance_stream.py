...
def run(test, params, env):
    ...
    # download stream tar ball
    session.cmd_status_output('uname -a')
    os_type = params["os_type"]

    tmp_dir = params.get("tmp_dir", "/tmp/")
    stream_deps_dir = data_dir.get_deps_dir("stream")
    stream_file = params.get('stream_file')
    stream_remote_path = os.path.join(stream_deps_dir, stream_file)
    stream_path = params.get('stream_path', tmp_dir)
    stream_src_path = os.path.join(stream_path, stream_file)

    vm.copy_files_to(stream_remote_path, stream_path)

    src_path = stream_src_path
    dst_path = tmp_dir
    stream_version = params.get('stream_version')
    stream = params.get('stream')
    stream_source_path = os.path.join(dst_path, stream)

    # compile stream
    compile_cmd = params["linux_compile_cmd"] % (src_path, dst_path, stream_source_path)
    logging.info("Compiling %s in guest..." % stream)
    status, output = session.cmd_status_output(compile_cmd)
    if status != 0:
        test.fail("Failed to compile stream with error: %s" % output)
    logging.debug(output)

    # run stream test: single thread
    test_cmd = params.get("single_thread_test_cmd")
    status, output = session.cmd_status_output("cd %s; %s " % (stream_source_path, test_cmd), timeout=3000)
    if status != 0:
        test.fail("Failed to run stream with error: %s" % output)
    logging.debug(output)

    # run stream test: multi thread                                                                                                                                                                                                                                                
    test_cmd = params.get("multi_thread_test_cmd")                                                                                                                                                                                                                                               
    status, output = session.cmd_status_output("cd %s; %s " % (stream_source_path, test_cmd), timeout=3000)                                                                                                                                                                         
    if status != 0:                                                                                                                                                                                                                                                                 
        test.fail("Failed to run stream with error: %s" % output)                                                                                                                                                                                                                   
    logging.debug(output)
